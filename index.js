...
var dream = function(fileId, name, ctx){
    return bot.telegram.getFileLink(fileId).then(fileLink => {
        if (fileLink) {
            DrmImg.dreamImage(fileLink, agent).then(result => {
                if (result === null) {
...
bot.hears(/\/dream/,ctx =>{
    ctx.reply(`Please send the image.`);
    ...
    bot.on('photo', ctx => {
        ...
        if (ctx.message.photo.length >= 2) {
            ...
            dream(fileId, 'result', ctx).catch(err => {
                logger.error(err);
    ...
    bot.on('document', ctx => {
        ...
        if (document) {
            ...
            logger.info(`[${ctx.message.from.id}] File id: ${fileId}`);
            dream(fileId, `broken-${document.file_name.replace(/\.[^/.]+$/, '')}`, ctx).catch(err => {
                logger.error(err);
                ctx.reply(`Failed to download image.`);
            });
        }
